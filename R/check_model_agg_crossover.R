...
check_model_agg_crossover <- function(data) {
  ...
  results_all <- data |>
    ...
    dplyr::filter(
      type %in% c("Multifactor Series", "Market Series", "Income Series")
    ) |>
    dplyr::mutate(
      violates_rule = dplyr::case_when(
        type == "Multifactor Series" &
          stringr::str_detect(model_agg, "MA Market|MA Income") ~ TRUE,
        type == "Market Series" &
          stringr::str_detect(model_agg, "MA Multifactor|MA Income") ~ TRUE,
        type == "Income Series" &
          stringr::str_detect(model_agg, "MA Market|MA Multifactor") ~ TRUE,
        .default = FALSE
      )
    ) |>
    dplyr::distinct()
