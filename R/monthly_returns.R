#' Calculate calendar month returns of a security
#'
...
#'
#'
monthly_returns <- function(x, start.date, end.date = Sys.Date()){

  PX_LAST <- price <- period <- last_day <- NULL
  ...
  # Trim to month-end values
  xx <- xx |>
    dplyr::mutate(period = date |> format("%B\n%Y")) |>
