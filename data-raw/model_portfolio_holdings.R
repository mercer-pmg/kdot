## code to prepare `model_portfolio_holdings` dataset goes here
...
process_env_holdings_export <- function(portfolio_name, exports_folder_path){
  ...
  explode_sleeves <- function(x, df = dat_sleeves){
    ...
    for(i in 1:length(y$name)){
      ...
      ch_ch_changes <- sleeve_holdings |>
        dplyr::filter(portfolio == y$name[i]) |>
        dplyr::filter(date >= y$start_date[i]) |>
        dplyr::filter(date < y$end_date[i]) |>
        dplyr::pull(date) |>
        unique()
...
model_portfolios <- exports |>
  dplyr::filter(type == "model portfolio") |>
  dplyr::pull(name)

model_portfolio_holdings <- model_portfolios |>
  purrr::map_df(
